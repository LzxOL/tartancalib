...
{
  echo "[stage6-no-final-ba-ablation] start: $(date)"
  echo "[stage6-no-final-ba-ablation] pid: $$"
  echo "[stage6-no-final-ba-ablation] train=${TRAIN_TAG} test=${TEST_TAG}"
  echo "[stage6-no-final-ba-ablation] mode=kalibr_style_batch valid_traversal pair_cohesion"
  echo "[stage6-no-final-ba-ablation] final_state=after_trial_selection"
  echo "[stage6-no-final-ba-ablation] output=${OUTPUT_DIR}"
  echo "[stage6-no-final-ba-ablation] cache=${CACHE_DIR}"
  ...
  echo "[stage6-no-final-ba-ablation] end: $(date)"
  echo "[stage6-no-final-ba-ablation] exit_code: ${exit_code}"
  exit "${exit_code}"
} >> "${LOG_FILE}" 2>&1
