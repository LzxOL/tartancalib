...
if [ "$#" -ne 5 ]; then
  echo "usage: $0 METHOD TEST_LEFT_DIR TEST_RIGHT_DIR TEST_LABEL OUTPUT_DIR" >&2
  echo "METHOD: no_cohesion_strict | current_baseline" >&2
  exit 2
fi
...
CACHE_DIR="result/.stage6_stereo_cache_134853_distance_${TEST_LABEL}_${METHOD}"
...
case "${METHOD}" in
  no_cohesion_strict)
    MODE_ARGS=(
      --stage6-pairboard-selection-mode strict_rmse
      --stage6-pair-selection-budget-mode fixed
      --stage6-pair-board-selection-budget-mode fixed
    )
    ;;
  current_baseline)
    MODE_ARGS=(
...
{
  echo "[stage6-134853-distance-split] start: $(date)"
  echo "[stage6-134853-distance-split] pid: $$"
  echo "[stage6-134853-distance-split] method=${METHOD}"
  echo "[stage6-134853-distance-split] test_label=${TEST_LABEL}"
  echo "[stage6-134853-distance-split] test_left=${TEST_LEFT_DIR}"
  echo "[stage6-134853-distance-split] test_right=${TEST_RIGHT_DIR}"
  echo "[stage6-134853-distance-split] output=${OUTPUT_DIR}"
  echo "[stage6-134853-distance-split] cache=${CACHE_DIR}"
  ./build/run_stage6_stereo_extrinsic "${COMMON_ARGS[@]}" "${MODE_ARGS[@]}"
  exit_code=$?
  echo "${exit_code}" > "${EXIT_FILE}"
  echo "[stage6-134853-distance-split] end: $(date)"
  echo "[stage6-134853-distance-split] exit_code: ${exit_code}"
  exit "${exit_code}"
} >> "${LOG_FILE}" 2>&1
