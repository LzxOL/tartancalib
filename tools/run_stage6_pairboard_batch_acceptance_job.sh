...
{
  echo "[stage6-pairboard-batch] start: $(date)"
  echo "[stage6-pairboard-batch] pid: $$"
  echo "[stage6-pairboard-batch] train=${TRAIN_TAG} test=${TEST_TAG} mode=kalibr_style_batch under_target_rescue"
  echo "[stage6-pairboard-batch] output=${OUTPUT_DIR}"
  echo "[stage6-pairboard-batch] cache=${CACHE_DIR}"
  ...
  echo "[stage6-pairboard-batch] end: $(date)"
  echo "[stage6-pairboard-batch] exit_code: ${exit_code}"
  exit "${exit_code}"
} >> "${LOG_FILE}" 2>&1
