...
This two-dimensional residual retains the two tangential degrees of
freedom of the ray discrepancy and locally approximates its angular
error. Since the observed ray depends on the camera parameters,
$\mathbf d^{\mathrm{obs}}_{t,b,j}$ and its tangent basis
$\mathbf B_{t,b,j}$ are updated during optimization.

We combine the pixel and ray residuals using the normalized hybrid
residual
\begin{equation}
...
\qquad \lambda\in[0,1],
\end{equation}
where $s_{\mathrm{px}}$ and $s_{\mathrm{ray}}$ are fixed scales
computed from the median residual magnitudes before final refinement.
The endpoint settings $\lambda=0$ and $\lambda=1$ correspond to
pixel-only and ray-only refinement, respectively. We use
$\lambda=0.5$ by default and examine its sensitivity in
Sec.~\ref{sec:lambda_sensitivity}.
