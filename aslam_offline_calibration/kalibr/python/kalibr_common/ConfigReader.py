...
class CalibrationTargetParameters(ParametersBase):
    ...
    def getTargetParams(self):
        ...
        if targetType == 'checkerboard':
            ...
            targetParams = {'tagRows': tagRows,
                            ...
                            'targetType': targetType}
        elif targetType == 'apriltag_internal':
            try:
                tagFamily = self.data["tagFamily"]
                tagId = self.data["tagId"]
                tagSize = self.data["tagSize"]
                blackBorderBits = self.data["blackBorderBits"]
                minVisiblePoints = self.data["minVisiblePoints"]
            except KeyError as e:
                self.raiseError("Calibration target configuration in {0} is missing the field: {1}".format(self.yamlFile, str(e)) )

            if not isinstance(tagFamily, str) or len(tagFamily) == 0:
                errList.append("invalid tagFamily (string)")
            if not isinstance(tagId, int) or tagId < 0:
                errList.append("invalid tagId (int>=0)")
            if not isinstance(tagSize, float) or tagSize <= 0.0:
                errList.append("invalid tagSize (float)")
            if not isinstance(blackBorderBits, int) or blackBorderBits < 0:
                errList.append("invalid blackBorderBits (int>=0)")
            if not isinstance(minVisiblePoints, int) or minVisiblePoints <= 0:
                errList.append("invalid minVisiblePoints (int>0)")

            targetParams = {'tagFamily': tagFamily,
                            'tagId': tagId,
                            'tagSize': tagSize,
                            'blackBorderBits': blackBorderBits,
    ...
    def printDetails(self, dest=sys.stdout):
        ...
        if targetType == 'checkerboard':        
            ...
            print("  Single tag:", file=dest)
            print("    Family: {0}".format(targetParams['tagFamily']), file=dest)
            print("    Id: {0}".format(targetParams['tagId']), file=dest)
            print("    Size: {0} [m]".format(targetParams['tagSize']), file=dest)
            print("    Border bits: {0}".format(targetParams['blackBorderBits']), file=dest)
            print("    Min visible points: {0}".format(targetParams['minVisiblePoints']), file=dest)
